...
from semilearn.datasets.cv_datasets.cub import get_cub
...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    from semilearn.datasets import get_eurosat

    if dataset == "eurosat":
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_eurosat(args, algorithm, 'eurosat', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}
        ...
        from semilearn.datasets import get_fgvc
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_fgvc(args, algorithm, 'fgvcaircraft', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}
    elif dataset == 'dtd':
        from semilearn.datasets import get_dtd
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_dtd(args, algorithm, 'dtd', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}

    elif dataset == 'flowers102':
        from semilearn.datasets import get_flowers
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_flowers(args, algorithm, 'flowers', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}

    elif dataset == 'resisc45':
        from semilearn.datasets import get_resisc
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_resisc(args, algorithm, 'resisc45', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}
        ...
        lb_dset, ulb_dset, eval_dset, test_dset, classnames = get_cub(args, algorithm, 'cub', num_labels, num_classes, data_dir, include_lb_to_ulb)
        return {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'classnames': classnames}
    else:
...
def get_optimizer(net, optim_name='SGD', lr=0.1, momentum=0.9, weight_decay=0, layer_decay=1.0, nesterov=True, bn_wd_skip=True, eps=1e-8):
    ...
    if hasattr(net, 'group_matcher') and layer_decay != 1.0:
    ...
    if not per_param_args:
        print("No parameter groups were defined by the model's group_matcher. "
              "Optimizing all parameters with requires_grad=True.")
        per_param_args = [p for p in net.parameters() if p.requires_grad]

    if optim_name == 'SGD':
        optimizer = torch.optim.SGD(per_param_args, lr=lr, momentum=momentum, weight_decay=weight_decay,
                                     nesterov=nesterov)
    elif optim_name == 'AdamW':
        optimizer = torch.optim.AdamW(per_param_args, lr=lr, weight_decay=weight_decay, eps=eps)
    else:
