...
def get_flowers(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):

    dataset_root = os.path.join(data_dir, 'Flowers102')
    ...
    print(f"Flowers-102: #Labeled: {len(lb_dset)} #Unlabeled: {len(ulb_dset)} #Val: {len(eval_dset)} #Test: {len(test_dset)}")

    return lb_dset, ulb_dset, eval_dset, test_dset, classnames

def get_flowers_tl(args, alg, name, num_classes, data_dir='./data'):
    dataset_root = os.path.join(data_dir, 'Flowers102')
    ...
    if 'flowers102' not in split_config:
        raise ValueError(f"Dataset key 'flowers102' not found in split.json.")

    seen_classes = set(split_config['flowers102']['seen'])
    unseen_classes = set(split_config['flowers102']['unseen'])
    ...
    print(f"[Flowers102 TL] Seen: {len(seen_classes)}, Unseen: {len(unseen_classes)}")
    print(f"[Flowers102 TL] #Labeled (Seen): {len(lb_dset)}, #Unlabeled (Unseen): {len(ulb_dset)}, #Test: {len(test_dset)}")
